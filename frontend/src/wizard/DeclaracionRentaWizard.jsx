...
import { crearDeclarante, crearPeriodo, actualizarPeriodo, calcularLiquidacion, listarPeriodos, obtenerParametrosPublicos, listarDeclarantes } from "../api.js";

// Hooks extraídos en DT-2 — estado del formulario y llamadas a la API
import { useWizardForm }  from "./useWizardForm.js";
import { useWizardApi }   from "./useWizardApi.js";
// ANIO_GRAVABLE, PARAMETROS_FALLBACK y PASOS se centralizan en wizardConfig.js
import { PASOS, ANIO_GRAVABLE, PARAMETROS_FALLBACK } from "./wizardConfig.js";
...
/* ------------------------------------------------------------------ */
/* WIZARD PRINCIPAL — refactorizado en DT-2                           */
/* Estado del formulario → useWizardForm                              */
/* Llamadas API e IDs   → useWizardApi                               */
/* Este componente: solo pasoActivo, navegación y render              */
/* ------------------------------------------------------------------ */
export default function DeclaracionRentaWizard({ sesion, declarante, onVolver, onSesionExpirada }) {

  const [pasoActivo, setPasoActivo] = useState(0);

  /* ---- Formulario, cálculos y validaciones ---- */
  // parametros se inicializa con el fallback; se actualiza cuando useWizardApi
  // carga los parámetros reales desde /configuracion/parametros-publicos/{anio}
  const [parametrosCargados, setParametrosCargados] = useState(PARAMETROS_FALLBACK);

  const form = useWizardForm(declarante, parametrosCargados);
  const {
    perfil, setPerfil,
    datos, setDatos,
  ...
  const {
    ...
    parametros,
    guardando, errorApi, setErrorApi,
  ...
  // Sincronizar parámetros cargados por la API hacia el hook de formulario
  useEffect(() => {
    if (parametros) setParametrosCargados(parametros);
  }, [parametros]);
